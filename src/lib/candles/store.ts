...
export async function ensureAndGet(
  ...
  fromTs: number,
  toTs: number,
  fetch1mRange: (fromTs: number, toTs: number) => Promise<CandlePoint[]>,
): Promise<CandlePoint[]> {
  const upper = symbol.toUpperCase();
  ...
  // If nothing found, try to backfill 1m for the range
  const baseInterval: Interval = '1m';
  const have1m = getCandles(upper, baseInterval, from, to);
  if (have1m.length === 0) {
    const fetched = await fetch1mRange(from, to);
  ...
  // If requested is not 1m, aggregate from 1m we now have
  if (reqInterval !== baseInterval) {
    const src1m = getCandles(upper, baseInterval, from, to);
    const agg = aggregateCandles(src1m, reqInterval);
    if (agg.length > 0) {
