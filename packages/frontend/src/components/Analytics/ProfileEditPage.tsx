...
export function ProfileEditPage({
  ...
  const [sources, setSources] = useState<DocumentSourcesListResponseDto | null>(
    null,
  );
  ...
  async function loadSources(profileId: string) {
    ...
    try {
      const data =
        await ProfilesService.profileControllerListSources(profileId);
      setSources(data);
      ...
      if (missing.length) {
        const fetched = await Promise.allSettled(
          missing.map((s) =>
            DocumentsService.documentsControllerGetOne(s.documentId),
          ),
        );
  ...
  async function assignDoc() {
    if (!id || !newDoc) return;
    try {
      await ProfilesService.profileControllerAssignSource(String(id), {
  ...
  async function unassignDoc(documentId: string) {
    if (!id) return;
    try {
      await ProfilesService.profileControllerUnassignSource(
        String(id),
        documentId,
      );
      await loadSources(String(id));
  ...
  async function save() {
    ...
    try {
      if (isNew) {
        const payload: ProfileCreateDto = {
          name,
  ...
  async function loadTask(profileId: string) {
    try {
      ...
      if (t?.taskId != null) {
        try {
          const full = await TasksService.tasksControllerGetOne(
            String(t.taskId),
          );
          setCurrentTask(full);
        } catch (e) {
    ...
    <Box>
      ...
      >
        ...
        <Typography variant="h6">
          {isNew
            ? 'Создать профиль'
            : `Редактировать профиль${profile ? `: ${profile.name}` : ''}`}
        </Typography>
      </Box>
        ...
        <>
              ...
              >
                <Box>
                  ...
                  <Typography variant="body2">
                    {currentTask?.name ?? `ID: ${currentTaskId}`}
                  </Typography>
          ...
          >
            ...
            <Button
              variant="contained"
              onClick={assignTask}
              disabled={!newTask || taskAssigning}
            >
          ...
          >
            ...
            <Button variant="contained" onClick={assignDoc} disabled={!newDoc}>
              Добавить
            </Button>
          </Box>
          ...
          <List dense sx={{ maxWidth: 800 }}>
            {sources?.items?.length
              ? sources.items.map((s: DocumentSourceDto) => {
                  ...
                  return (
                    <ListItem
                      ...
                      secondaryAction={
                        <IconButton
                          edge="end"
                          aria-label="remove"
                          onClick={() => unassignDoc(s.documentId)}
                          title="Убрать"
                        >
                          <Delete />
                        </IconButton>
                    ...
                    >
                      <ListItemText
                        primary={
                          d ? d.title || d.id : `Документ ${s.documentId}`
                        }
                        secondary={
                          d
                            ? `Скрапер: ${d.scraper?.name ?? d.scraper?.id ?? '—'} • Извлечено: ${new Date(d.scrapedAt).toLocaleString()}`
                            : 'Загрузка деталей...'
                        }
                      />
                    </ListItem>
              ...
              : !docsLoading && (
                  <Typography variant="body2">Документы не выбраны</Typography>
                )}
          </List>
        </>
      )}
