...
export default function registerTradesImportTinkoff(
  jobsProcessor: JobsProcessor,
) {
  jobsProcessor.register('trades.import.tinkoff', async (job) => {
    ...
    const singleTicker =
      (payload.ticker || payload.symbol || '').trim?.() || '';
    ...
    const hours =
      Number.isFinite(hoursN) && hoursN > 0 ? Math.floor(hoursN) : 24;
