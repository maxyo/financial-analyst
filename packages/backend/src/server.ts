...
// Static frontend (serve the frontend directory which contains index.html and build/ bundle)
...
// ---- Jobs API ----
const jobsProcessor = new JobsProcessor({ intervalMs: Number(process.env.JOBS_POLL_MS || 1000) });
// Register a default 'noop' handler that just echoes payload
...
app.post('/api/jobs/:id/cancel', (req: Request, res: Response) => {
  try {
    ...
    const job = getJobStatus(id);
    if (!job) return res.status(404).json({ error: 'not found' });
    if (job.status === 'running') return res.status(400).json({ error: 'cannot cancel running job' });
...
// Trades endpoint: return user's own trades by default (last 24h); fallback to frontend if requested
app.get('/api/trades', async (req: Request, res: Response) => {
  try {
    ...
    // If explicit mode=frontend passed, return anonymized market trades for backward-compat
    ...
    // Best-effort fallback to frontend anonymized if user operations are unavailable
    try {
      const ticker = (getQ(req, 'ticker') || '').trim();
      const trades = await getRecentTradesByTicker(ticker);
