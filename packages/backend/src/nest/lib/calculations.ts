import { Helpers } from '../modules/market-prodivers/tinkoff';

import type { Quotation } from '../modules/market-prodivers/tinkoff';
...
export function computeMoexClearingInstants(date?: Date): string[] {
  ...
  return [new Date(Date.UTC(y, m, d, 16, 0, 0, 0)).toISOString()];
}
...
export function fundingRateEstAt(
  ...
  for (const p of points) {
    const tStr = (p as PricePointLike).t;
